...
# Initialize transition matrix as a list of sparse matrices
transition_matrix = [
    lil_matrix((len(ex_space) * len(ey_space) * len(eth_space) * len(v_space) * len(w_space), 8), dtype=np.float32)
    for _ in range(nt)
]
...
def compute_transition_matrix_for_time_step(t, ex_space, ey_space, eth_space, v_space, w_space, delta_t, sigma):
    local_transition_matrix = lil_matrix((len(ex_space) * len(ey_space) * len(eth_space) * len(v_space) * len(w_space), 8), dtype=np.float32)
    index_map = np.zeros((len(ex_space), len(ey_space), len(eth_space), len(v_space), len(w_space)), dtype=int)
    counter = 0
            ...
            for it in range(len(eth_space)):
                for iv in range(len(v_space)):
                    for iw in range(len(w_space)):
                        index_map[ix, iy, it, iv, iw] = counter
                        counter += 1
                ...
                for iv, v in enumerate(v_space):
                    for iw, w in enumerate(w_space):
                        ...
                        for idx, (ni, nj, nk) in enumerate(neighbors):
                            ...
                            current_idx = index_map[ix, iy, it, iv, iw]
                            local_transition_matrix[current_idx, idx] = prob
        ...
        for ix in range(len(ex_space)):
            for iy in range(len(ey_space)):
                for it in range(len(eth_space)):
                    for iv in range(len(v_space)):
                        for iw in range(len(w_space)):
                            current_idx = ix * len(ey_space) * len(eth_space) * len(v_space) * len(w_space) + iy * len(eth_space) * len(v_space) * len(w_space) + it * len(v_space) * len(w_space) + iv * len(w_space) + iw
                            total_prob = np.sum(transition_matrix[t][current_idx, :])
                            if total_prob > 0:
                                transition_matrix[t][current_idx, :] /= total_prob
