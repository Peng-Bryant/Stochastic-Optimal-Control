...
def compute_transition_matrix_for_state_control(t, ix, ex, iy, ey, it, eth, iv, v, iw, w, delta_t, sigma, ex_space, ey_space, eth_space):
    ...
    if total_prob > 0:
        print(total_prob)
        local_transition_matrix[:, 3] /= total_prob
...
def compute_transition_matrix(transition_matrix, ex_space, ey_space, eth_space, v_space, w_space, delta_t, sigma):
                ...
                for it, eth in enumerate(eth_space):
                    for iv, v in enumerate(v_space):
                        for iw, w in enumerate(w_space):
                            tasks.append((t, ix, ex, iy, ey, it, eth, iv, v, iw, w, delta_t, sigma, ex_space, ey_space, eth_space))
    ...
    end_time = time.time()
    print(f"Time taken for updating transition matrix: {end_time - start_time:.2f} seconds")
# Example usage
