...
class GPI:
    ...
    def evaluate_batch(self, time_range):
        ...
        for t in time_range:
            if t + 1 >= self.nt:
                continue  # Skip out-of-bound time steps
    ...
    def policy_evaluation(self):
        ...
        for _ in range(self.config.num_evals):
            time_ranges = [range(t, t + self.batch_size) for t in range(self.nt - 2, -1, -self.batch_size)]
            results = Parallel(n_jobs=-1)(
                delayed(self.evaluate_batch)(time_range) for time_range in tqdm.tqdm(time_ranges, desc="Policy Evaluation")
            )
            
            for new_value_function in results:
                self.config.V.copy_from(new_value_function)

    def improve_policy_batch(self, time_range):
        results = []
        ex_indices, ey_indices, eth_indices = np.meshgrid(
            np.arange(len(self.config.ex_space)),
            np.arange(len(self.config.ey_space)),
    ...
    def policy_improvement(self):
        ...
        time_ranges = [range(t, t + self.batch_size) for t in range(0, self.nt - 1, self.batch_size)]
        results = Parallel(n_jobs=-1)(
            delayed(self.improve_policy_batch)(time_range) for time_range in tqdm.tqdm(time_ranges, desc="Policy Improvement")
        )
...
if __name__ == "__main__":
    ...
    num_evals = 20  # Number of policy evaluations in each iteration
    ...
    data_folder = './data/'
    gpi = GPI(config, nt, batch_size=50)  # Adjust the batch size as needed
