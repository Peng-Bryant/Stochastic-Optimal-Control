...
class GPI:
    ...
    def log_value_function(self, iteration):
        values = self.config.V(0, self.config.ex_space, self.config.ey_space, self.config.eth_space)
        import pdb; pdb.set_trace()
        if not os.path.exists(self.config.output_dir):
            os.makedirs(self.config.output_dir)
        np.savez(f'{self.config.output_dir}/value_function_iter_{iteration}.npz', values=values)

    def log_policy(self, iteration):
        if not os.path.exists(self.config.output_dir):
            os.makedirs(self.config.output_dir)
        np.savez(f'{self.config.output_dir}/policy_iter_{iteration}.npz', policy=self.policy)
...
if __name__ == "__main__":
    ...
    output_dir = './output/toy'  # Output directory for saving results
