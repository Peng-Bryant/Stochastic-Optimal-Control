...
class GPI:
    ...
    def policy_evaluation(self):
        ...
        for _ in range(self.config.num_evals):
            ...
            for t in range(self.nt - 2, -1, -1):
                ...
                batches = [indices[i:i + batch_size] for i in range(0, len(indices), batch_size)]  # Batch size of 100

                results = Parallel(n_jobs=-1)(
                    delayed(self.evaluate_batch)(t, batch) for batch in batches
                )
    ...
    def policy_improvement(self):
        ...
        for t in range(self.nt - 1):
            ...
            results = Parallel(n_jobs=-1)(
                delayed(self.improve_policy_batch)(t, batch) for batch in batches
            )
